...
export default function AddPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/purchased-customers', {
        method: 'POST',
      ...
      if (res.ok) {
        setMessage({ text: 'Customer data has been successfully synchronized.', type: 'success' });
        setForm({
          date: new Date().toISOString().split('T')[0],
          customerName: '',
          item: 'Rug',
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="bg-dark-surface border border-dark-border rounded-2xl shadow-2xl overflow-hidden">
        <div className="p-8 border-b border-dark-border bg-gradient-to-br from-dark-surface to-zinc-900">
          <h2 className="text-3xl font-extrabold text-white">Add New Record</h2>
          <p className="text-gray-500 mt-1">Enter customer transaction details below.</p>
        </div>

        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className={labelClasses}>Date of Transaction</label>
              <input
                type="date"
                value={form.date}
          ...
          <div>
            <label className={labelClasses}>Full Customer Name</label>
            <input
              type="text"
