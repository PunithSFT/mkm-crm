...
export default function SearchPage() {
  ...
  useEffect(() => {
    ...
    const result = customers.filter(c =>
      ...
      c['Short Description']?.toLowerCase().includes(term)   // ← optional: also search in description
    );
    setFiltered(result);
    ...
    <div className="max-w-7xl mx-auto space-y-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-dark-surface p-6 rounded-2xl border border-dark-border shadow-xl">
        <div>
          ...
          <p className="text-gray-500 mt-1">Review and manage all customer transaction records.</p>
        </div>
        <button
      ...
      <div className="relative group">
        ...
        <input
          type="text"
          placeholder="Search by name, telephone, or reference..."
          value={searchTerm}
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-zinc-900/50 border-b border-dark-border">
                ...
                <th className="p-5 text-xs font-bold text-gray-400 uppercase tracking-widest">Short Description</th>
              </tr>
            </thead>
